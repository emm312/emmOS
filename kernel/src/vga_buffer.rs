...
lazy_static! {
    pub static ref WRITER: Mutex<Writer> = Mutex::new(Writer::new(
        unsafe { globals::FRAMEBUFFER.lock().assume_init_read() },
        unsafe { globals::FRAMEINFO.lock().assume_init_read() }
    ));
}
...
impl Writer {
    ...
    fn newline(&mut self) {
        self.y_pos += font_constants::CHAR_RASTER_HEIGHT.val() + VERT_SPACE;
    ...
    fn write_rendered_char(&mut self, rendered_char: RasterizedChar) {
        ...
        self.x_pos += rendered_char.width() + LETTER_SPACE;
    }
